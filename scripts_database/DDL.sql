...
BEGIN
    ...
    DATE_FORMAT(fecha_registro, '%d-%c-%y') AS 'fecha_registro',
    DATE_FORMAT(fecha_registro, '%h:%i:%s') AS 'hora_registro',
    DATE_FORMAT(fecha_modificacion, '%d-%c-%y') AS 'fecha_modificacion',
...
BEGIN
	...
	DATE_FORMAT(Tarea.fecha_registro, '%d-%c-%y') AS '_fecha_registro', DATE_FORMAT(Tarea.fecha_registro, '%h:%i:%s') AS '_hora_registro',
	DATE_FORMAT(Tarea.fecha_final, '%d-%c-%y') AS '_fecha_final', DATE_FORMAT(Tarea.fecha_final, '%h:%i:%s') AS '_hora_final'
	FROM UsuarioTareas
	INNER JOIN TAREA ON (UsuarioTareas.idTarea = Tarea.idTarea) WHERE idUsuario = _idUsuario ORDER BY _fecha_final AND _hora_final DESC;
END $$
DELIMITER ;